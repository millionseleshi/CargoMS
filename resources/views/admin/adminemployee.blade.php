        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('AdminAddEmployee') }}">
                            {{ csrf_field() }}


                            <div class="form-group row">

                                <label for="employeeType" class="col-md-auto col-form-label text-md-right">{{ __('Forwarder') }}</label>

                                <div class="col-md-3">
                                    <input id="employeeForwarder" type="radio" class="form-control icheckbox_flat-aero" name="employeeType" value="forwarder" required >
                                </div>

                                <label for="employeeType" class="col-md-auto col-form-label text-md-right">{{ __('Deliverer') }}</label>

                                <div class="col-md-3">
                                    <input id="employeeDeliverer" type="radio"  class="form-control icheckbox_flat-aero" name="employeeType" value="deliverer" required >
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                <label for="organizationList" class="col-md-4 col-form-label text-md-right">{{ __('Organization') }}</label>

                                <div class="col-md-6" id="forwarderBox" hidden="hidden">
                                    <select id="organizationList" class="form-control select2 {{ $errors->has('organizationList') ? ' is-invalid' : '' }}" name="forwarderList" value="{{ old('organizationList') }}" required autofocus style="width: 100%;">
                                        @foreach($forwarderOrganization as $value)
                                            <option value="{{$value->id}}">{{$value->companyName}}</option>
                                        @endforeach
                                    </select>

                                    @if ($errors->has('organizationList'))
                                        <span class="invalid-feedback" role="alert">
                                        <strong>{{ $errors->first('organizationList') }}</strong>
                                    </span>
                                    @endif
                                </div>

                                <div class="col-md-6" id="delivererBox" hidden="hidden">
                                    <select id="organizationList" class="form-control  select2 {{ $errors->has('organizationList') ? ' is-invalid' : '' }}" name="delivererList" value="{{ old('organizationList') }}" required autofocus style="width: 100%;">
                                        @foreach($delivererOrganization as $value)
                                            <option value="{{$value->id}}">{{$value->companyName}}</option>
                                        @endforeach
                                    </select>
